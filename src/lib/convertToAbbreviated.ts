 ...
 */
export const  convertToAbbreviated= (value: string | number): string => {
    ...
    if (numericValue >= 1_000_000_000) {
        return (numericValue / 1_000_000_000).toFixed(2).replace(/\.0$/, '') + 'B';
    } else if (numericValue >= 1_000_000) {
        return (numericValue / 1_000_000).toFixed(2).replace(/\.0$/, '') + 'M';
